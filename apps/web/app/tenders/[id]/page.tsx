...
export default function TenderPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [evidence, setEvidence] = useState<{ field: string; refs: EvidenceReference[] } | null>(null);
  const [sourceBlock, setSourceBlock] = useState<DocumentBlock | null>(null);
  ...
  const processing = tender?.documents.some((d) => d.ingestion_status === "processing") ?? false;
  useEffect(() => {
    if (!processing) return;
    const timer = setInterval(async () => {
      setTender(await api<Tender>(`/api/tenders/${id}/refresh/`, { method: "POST" }));
    }, 3000);
  ...
  async function showEvidence(field: string, refs: EvidenceReference[]) {
    setEvidence({ field, refs });
    setSourceBlock(null);
    const ref = refs[0];
    if (!ref?.block_id) return;
    // Resolve the platform document owning this engine document for the block fetch.
    const doc = tender?.documents.find((d) => d.engine_document_id === ref.document_id);
    if (!doc) return;
    try {
      const blocks = await api<DocumentBlock[]>(`/api/documents/blocks/${doc.id}/`);
      setSourceBlock(blocks.find((b) => b.id === ref.block_id) ?? null);
    } catch {
      setSourceBlock(null);
    }
  }
  ...
  return (
    <div>
      <h1>
        ...
        {tender.decision && <span className={`badge ${tender.decision}`}>{tender.decision.replace("_", " ")}</span>}
      </h1>
      ...
      <div className="card">
        ...
        <div style={{ marginTop: 12 }}>
          ...
          />
          {processing && <p className="muted">Processing… the AI Engine is parsing and extracting.</p>}
        </div>
      </div>
                ...
                <div className="meta-field" key={key}>
                  ...
                  {hasValue && field!.evidence.length > 0 && (
                    <a
                      ...
                      onClick={(e) => {
                        e.preventDefault();
                        void showEvidence(label, field!.evidence);
                      }}
                      href="#evidence"
                    >
            ...
            {evidence.refs.map((r, i) => (
              <div key={i}>
                ...
                <p className="src">
                  ...
                  {r.verified ? " · quote verified against source" : " · quote NOT verified"}
                </p>
              </div>
            ))}
            {sourceBlock && (
              <div className="src">
                <strong>Source block:</strong> {sourceBlock.text}
              </div>
            )}
          </div>
        )}
      ...
      <div className="card">
        ...
        {tender.decision ? (
          <p>
            Decision: <span className={`badge ${tender.decision}`}>{tender.decision.replace("_", " ")}</span> —{" "}
